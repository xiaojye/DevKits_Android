...
public abstract class PreviewBuilder<T extends PreviewBuilder> {
    ...
    public PreviewBuilder(DkImagePreview preview) {
        mPreviewOptions = PreviewOptions.getCleanInstance();
        mPreviewOptions.context = preview.getContext().get();
    }
     ...
     * @param loader 加载引擎
     */
    public void start(DkImagePreview.ILoader loader) {
        ...
        if (loader == null) {
            throw new NullPointerException("loader is null, please call " + DkImagePreview.class.toString() + "$initLoader before use.");
        }
