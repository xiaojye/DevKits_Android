 ...
 * @since 1.0
 */
public class DkLetterIndexBar extends View {
    ...
    private float mOverlayTextSize = DisplayUtils.sp2px(getContext(), 40f);
    //提示框宽度（单位：dp），默认70dp
    private float mOverlayWith = DisplayUtils.dp2px(getContext(), 70f);
    //提示框高度（单位：dp），默认70dp
    private float mOverlayHeight = DisplayUtils.dp2px(getContext(), 70f);
    ...
    //字母字体大小（单位：sp），默认9sp
    private float mLetterTextSize = DisplayUtils.sp2px(getContext(), 10f);
    //字母之间的间隔（单位：dp），默认2dp
    private float mLetterTextSpace = DisplayUtils.dp2px(getContext(), 3f);
    ...
    public DkLetterIndexBar(Context context) {
        super(context);
        init(null);
    }

    public DkLetterIndexBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public DkLetterIndexBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = getResources().obtainAttributes(attrs, R.styleable.DkLetterIndexBar);

            if (typedArray.hasValue(R.styleable.DkLetterIndexBar_dk_lib_textColor)) {
                mLetterTextColor = typedArray.getColor(
                        R.styleable.DkLetterIndexBar_dk_lib_textColor, Color.parseColor("#157DF9"));
            }
